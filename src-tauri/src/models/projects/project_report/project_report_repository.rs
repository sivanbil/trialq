...
use crate::models::projects::project_report::schema::project_reports::dsl::*;
use diesel::prelude::*;
...
impl ProjectReportRepository {
    ...
    pub fn create_report(&self, new_report: NewProjectReport) -> Result<ProjectReport, String> {
        ...
        diesel::insert_into(project_reports)
            .values(&new_report)
            .execute(&mut conn)
    ...
    pub fn update_report(&self, report_id: i32, updated_report: NewProjectReport) -> Result<usize, String> {
        let mut conn = self.pool.get().map_err(|e| e.to_string())?;
        diesel::update(project_reports.find(report_id))
            .set(&updated_report)
            .execute(&mut conn)
            .map_err(|e| e.to_string())
    }
    ...
    pub fn delete_report(&self, report_id: i32) -> Result<usize, String> {
        let mut conn = self.pool.get().map_err(|e| e.to_string())?;
        diesel::delete(project_reports.find(report_id))
            .execute(&mut conn)
            .map_err(|e| e.to_string())
    }
}
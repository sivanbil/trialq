...
use crate::models::projects::project_report::project_report_source_model::{ProjectReportSource, NewProjectReportSource};
...
impl ProjectReportSourceRepository {
    ...
    pub fn create(&self, new_source: NewProjectReportSource) -> Result<ProjectReportSource, diesel::result::Error> {
    ...
    pub fn find_by_id(&self, source_id: i32) -> Result<Option<ProjectReportSource>, diesel::result::Error> {
        let mut conn = self.pool.get().unwrap();
        project_report_source.find(source_id).first(&mut conn).optional()
    }
    ...
    pub fn update(&self, source_id: i32, updated_source: NewProjectReportSource) -> Result<usize, diesel::result::Error> {
        let mut conn = self.pool.get().unwrap();
    ...
    pub fn delete(&self, source_id: i32) -> Result<usize, diesel::result::Error> {
        let mut conn = self.pool.get().unwrap();
        diesel::delete(project_report_source.find(source_id)).execute(&mut conn)
    }
}